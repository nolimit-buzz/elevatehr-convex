...
import { useEmailTemplates, useEmailTemplateMutations, TemplateType } from "@/queries/emailTemplates.queries";
...
const TEMPLATE_LABELS: Record<string, string> = {
  ...
  online_assessment_1: "Online Assessment 1",
  online_assessment_2: "Online Assessment 2",
...
const EmailTemplatePage = () => {
  ...
  const templateType = (searchParams.get("type") || "skill_assessment") as TemplateType;

  const templatesData = useEmailTemplates();
  const mutations = useEmailTemplateMutations();
  ...
  useEffect(() => {
    if (templates && templates[templateType]) {
      setTemplate({
        subject: templates[templateType].subject ?? "",
        content: templates[templateType].content ?? "",
      });
    } else {
      setTemplate({ subject: "", content: "" });
    }
  }, [templateType, templatesData]);
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      const { error } = await mutations.upsertTemplate(templateType, template.content, template.subject);
    ...
    <Box sx={{ maxWidth: "1280px", mx: "auto" }}>
      ...
      <Box sx={{ display: "flex", mt: 3 }}>
        <Box sx={{ width: "30%", minWidth: "300px", maxWidth: "416px" }}>
          ...
          >
            <Typography variant="h6" sx={{ p: 2, borderBottom: "0.8px solid rgba(17, 17, 17, 0.08)" }}>
              Templates
            </Typography>
            <List>
          ...
          >
            <Stack spacing={3}>
              ...
              <Box sx={{ display: "flex", gap: 2, justifyContent: "flex-end" }}>
                <PrimaryButton variant="contained" onClick={handleSave} disabled={saving}>
                  {saving ? (
                    <>
                      {" "}
                      <Typography variant="body2" sx={{ fontSize: "16px", fontWeight: 600, color: "secondary.light" }}>
                        Saving
                      </Typography>
